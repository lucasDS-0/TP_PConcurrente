...
public class ConjuntoGranularidadGruesa<T> implements Conjunto<T>{
    ...
    public static class Factory<T> implements Conjunto.Factory<T>{
        @Override public Conjunto<T> armarConjuntoVacio(Comparator<T> comparator){
            return new ConjuntoGranularidadGruesa<T>(comparator);
        }
    }

    private ConjuntoGranularidadGruesa(Comparator<T> comparator){
	...
	@Override public synchronized boolean agregar(T elementoNuevo){
	...
	@Override public synchronized boolean remover(T elemento){
	    Nodo<T> nodoAnt = this.marcaDeInicio;
